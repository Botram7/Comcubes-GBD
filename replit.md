...
- **Payment Gateway**: Dual support for Paystack (primary via Zenith Bank USD account) and PayPal (secondary) for USD transactions. Feature-flagged NGN fallback system preserved for emergency use (requires manual activation via `PAYSTACK_ENABLE_NGN_FALLBACK=true`).
  - **Payment Channels**: Currency-aware channel selection - USD supports card only, NGN supports card/bank/USSD/QR/mobile money/bank transfer. Other currencies use Paystack auto-selection.
  - **Apple Pay**: Domain verification file preloaded at startup and served with text/plain content-type. Requires app restart to reload if file changes.
