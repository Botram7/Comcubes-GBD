...
- **GDPR Cookie Consent**: Custom cookie consent banner with Accept All/Reject/Preferences options. Analytics (GA4, Clarity) and Marketing (AdSense) cookies only load after explicit user consent. Consent preferences stored in localStorage. Revoking consent after scripts are loaded prompts page reload for full compliance.
- **Anti-Spam Protection**: Cloudflare Turnstile integration on contact form for bot protection. Privacy-friendly CAPTCHA alternative that verifies human users without tracking. Server-side token verification before processing form submissions.
