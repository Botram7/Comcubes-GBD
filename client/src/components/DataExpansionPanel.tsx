...
export function DataExpansionPanel() {
  ...
  const generateMutation = useMutation({
    mutationFn: async (params: { industryName: string; sectorName: string; geoTarget?: string }) => {
      const response = await apiRequest('POST', '/api/admin/ai-generator/generate', params);
      return response.json();
    },
    onSuccess: (data) => {
      setGeneratedResults(data.companies || []);
      toast({ title: 'Generation complete', description: `Generated ${data.companies?.length || 0} company suggestions` });
    },
    onError: (error: Error) => {
  ...
  const enrichMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({ title: 'Enrichment complete', description: `Enriched ${successCount} descriptions, ${data.remaining} remaining` });
    },
    onError: (error: Error) => {
  ...
  const wikidataMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast({ title: 'Wikidata search complete', description: `Found ${data.companies?.length || data.length || 0} companies` });
    },
    onError: (error: Error) => {
  ...
  const importMutation = useMutation({
    mutationFn: async (params: { companies: any[]; industryName: string; sectorName: string }) => {
      const response = await apiRequest('POST', '/api/admin/ai-generator/import', params);
      return response.json();
    },
    onSuccess: (data) => {
      toast({ title: 'Import complete', description: `Imported ${data.imported || 0} companies` });
      setGeneratedResults([]);
    },
    onError: (error: Error) => {
            ...
            <Button
              onClick={() => {
                if (generatingIndustry) {
                  const gap = gaps?.find(g => g.industryName === generatingIndustry);
                  if (gap) {
                    ...
                    <TableRow key={gap.industryName} className={generatingIndustry === gap.industryName ? 'bg-blue-50' : ''}>
                      ...
                      <TableCell>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setGeneratingIndustry(gap.industryName)}
                        >
                          Select
                        </Button>
                      </TableCell>
              ...
              <TableBody>
                {generatedResults.map((company: any, i: number) => (
                  <TableRow key={i}>
                    ...
                    <TableCell className="text-sm text-blue-600">{company.websiteUrl || company.website}</TableCell>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-gray-500">
            ...
            Wikidata is completely free. Queries their open SPARQL endpoint - no API key needed.
          </p>
        </CardContent>
      </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">
              Wikidata Results ({(wikidataMutation.data as any).companies?.length || (wikidataMutation.data as any).length || 0})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="max-h-96 overflow-y-auto">
              <Table>
                ...
                <TableBody>
                  {((wikidataMutation.data as any).companies || wikidataMutation.data as any[] || []).map((company: any, i: number) => (
                    <TableRow key={i}>
